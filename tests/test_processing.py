...
@pytest.mark.parametrize("state, expected_count, description", [
    ("EXECUTED", 3, "Должно вернуть 3 выполненные транзакции"),
    ("CANCELED", 2, "Должно вернуть 2 отмененные транзакции"),
    ])
def test_filter_by_state_valid(transactions_mixed: list[dict], state: str, expected_count: int, description: str)->None:
...
@pytest.mark.parametrize("reverse, expected_first_date, expected_last_date", [
    (True, "2024-01-25T11:00:00", "2024-01-05T16:45:00"),
    (False, "2024-01-05T16:45:00", "2024-01-25T11:00:00"),
])
def test_sort_by_date_parametrized(transactions_unsorted: list[dict], reverse: bool, expected_first_date: str, expected_last_date: str) -> None:
